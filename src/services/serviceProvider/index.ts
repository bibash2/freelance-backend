...
class ServiceProviderService {
    ...
    async createServiceProvider(payload: any) {
        try {
            ...
            const serviceProvider = await prisma.user.update({data: 
    ...
    async getServiceProviderList(payload: any) {
        try {
            ...
            if(category){
                allServiceProvider.filter((a:any) => a.category === category)
            ...
            const allServiceProviderWithDistance = allServiceProvider.map((a:any) => {
            ...
            if(category){
                allServiceProviderWithDistance.filter((a:any) => a.distance <= 3)
            }
    ...
    async contactServiceProvider(payload: any) {
        try {
            const {userId, serviceProviderId, title, description, address, datetime} = payload;
            const {latitude, longitude} = await geoEncodingService.convertAddressToGeocode(address);
            const serviceProvider = await prisma.workCall.create({data: {serviceProviderId, userId, chatHistory: {message: payload.message}, title, description, address, location: {latitude, longitude}, dateTime: datetime}})
            return serviceProvider;
        } catch (error) {
            throw error;
        }
